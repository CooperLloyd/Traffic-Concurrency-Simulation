...
public class Car extends Thread {
  ...
  public void run () {
    for (int I=1;  I<= 4; I++) {
      ...
      // Increase west queue one at a time
      Synch.westCarMutex.acquire();
      Synch.westBoundCarCount ++;
      Synch.westCarMutex.release();

      // Wait until released by green light
      Synch.westQueue.acquire();

      // Count each successful acquire()
      Synch.westAqMutex.acquire();
      Synch.westAqCount ++;
      Synch.westAqMutex.release();


      // If Westbound light is green, Lights thread releases car
      Synch.westGreenMutex.acquire();
      if(Synch.lightStatus == 1){
        System.out.println("At time " + Synch.timeSim.curTime() + " Car " + myName + " is starting to cross westbound.\n");
        Synch.westCarMutex.acquire();
        Synch.westBoundCarCount--;
        Synch.westCarMutex.release();
        Synch.timeSim.doSleep(100); // Start crossing
      }
      ...
      // Increase east queue one at a time
      Synch.eastCarMutex.acquire();
      Synch.eastBoundCarCount ++;
      Synch.eastCarMutex.release();

      // Wait until released by green light
      Synch.eastQueue.acquire();

      // Count each successful acquire()
      Synch.eastAqMutex.acquire();
      Synch.eastAqCount ++;
      Synch.eastAqMutex.release();

      // If eastbound light is green, Lights thread releases car
      Synch.eastGreenMutex.acquire();
      if(Synch.lightStatus == 2){
        ...
        Synch.timeSim.doSleep(100); // Start crossin
...
// Bug: when east light is green and the car count goes to 0,
// program sleeps for 1 before releasing car across causeway